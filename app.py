...
if uploaded_files:
    ...
    if df_all is not None and not df_all.empty:
        ...
        # Chỉ giữ đúng 1 bộ chọn tháng/năm TỔNG QUAN duy nhất ở trên cùng để nạp dữ liệu
        ky_list = sorted(df_all['Kỳ_Tháng'].unique())
        selected_kys = st.multiselect("🗓️ Chọn dữ liệu Tháng/Năm muốn đưa vào phân tích (Ảnh hưởng đến tất cả các Tab):", ky_list, default=ky_list)
        ...
        if not selected_kys:
            ...
            # ----------------------------------------------------
            # 🆕 TAB 3: ĐÃ XÓA SẠCH BỘ LỌC RƯỜM RÀ, DÙNG BỘ LỌC CỦA BẢNG
            # ----------------------------------------------------
            with tab3:
                st.markdown("### 📋 BÁO CÁO CHI TIẾT (SỬ DỤNG BỘ LỌC TRỰC TIẾP TRÊN CỘT DỮ LIỆU)")
                st.info("💡 **HƯỚNG DẪN LỌC:** Hãy rê chuột vào tiêu đề của bất kỳ cột nào trong bảng (VD: 'Năm' hoặc 'Vật tư'), bạn sẽ thấy biểu tượng **Kính lúp (Search)** hiện ra ở góc phải. Bấm vào đó để lọc dữ liệu y hệt Excel!")
                ...
                # Cảnh báo minh bạch cho người dùng
                st.warning("⚠️ **Lưu ý Kỹ thuật:** 3 con số Tổng dưới đây là số tổng của toàn bộ các tháng bạn đã chọn. Nếu bạn sử dụng Kính lúp lọc bên trong bảng, 3 con số này sẽ **KHÔNG** tự động nhảy theo (Vì bảng chỉ giấu dữ liệu đi chứ không tính toán lại).")
                ...
                col3.markdown(f"""<div class="metric-card"><h4>⚙️ TỔNG SỐ MÃ SP</h4><h2 style="color:#2E7D32;">{df_display['Vật tư'].nunique()} Mã</h2></div>""", unsafe_allow_html=True)
                ...
                # Bảng thuần khiết với bộ lọc Kính lúp tàng hình
                ...
                st.download_button(label="📥 TẢI BÁO CÁO CỦA BẢNG NÀY (File CSV)", data=csv_data, file_name='Bao_Cao_Chi_Tiet.csv', mime='text/csv')
